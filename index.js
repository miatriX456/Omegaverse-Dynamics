...
// ============================================================
// 4. ФУНКЦИИ ДЛЯ РАБОТЫ С ЗАПАХАМИ
// ============================================================
...
// ============================================================
// 5. ПАРСЕР СООБЩЕНИЙ (АВТОМАТИЧЕСКОЕ РАСПОЗНАВАНИЕ)
// ============================================================
...
function applyDetectedChanges(detected) {
    ...
    if (detected.status && detected.newNpc) {
        ...
        if (npc && npc.status !== detected.status) {
            ...
            npc.lastUpdate = Date.now();  // <-- ДОБАВЛЯЕМ
...
// Автоматическое добавление фактов
if (detected.newNpc) {
...
if (detected.cycle && detected.newNpc) {
    const cycleType = detected.cycle === 'heat' ? 'течка' : 'гон';
    addFact(`У ${detected.newNpc} начался ${cycleType}`, 'events', 'medium');
}
...
// ============================================================
// 6. ГЕНЕРАЦИЯ СИСТЕМНОГО ПРОМПТА
// ============================================================
...
// ============================================================
// 8. РЕНДЕРИНГ UI (вкладки, карточки, настройки)
// ============================================================
...
function renderNpcListContent() {
    ...
    if (state.npcs.length === 0) {
        ...
        state.npcs.forEach((npc, index) => {
            ...
            // Проверяем, обновлялся ли NPC за последние 10 секунд
...
function renderCalendarContent() {
    ...
    // Парсим текущий месяц и год для отображения
    ...
    let month = parseInt(parts[1]) - 1; // 0-based для JS Date
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        // Собираем имена NPC с событиями в этот день
        let eventList = [];
        for (const npc of state.npcs) {
            if (npc.cycle.active && npc.cycle.startDate === dateStr) {
    ...
    // Блок с событиями выбранного дня (по умолчанию показываем сегодня)
    html += `
    ...
    // Добавляем обработчики навигации (они будут назначены после рендера)
    setTimeout(() => {
...
jQuery(async () => {
    try {
        ...
        // Добавляем настройки в меню расширений
        ...
        // Обработчики для настроек в меню расширений
...
$(document).on('click', '.ov-edit-btn', function() {
    ...
    notify(`Данные ${npc.name} сохранены`, 'system', 'fa-solid fa-check-circle');
        ...
        console.log('[OV] Omegaverse Dynamics v0.5.0 готово! (с автообнаружением)');
    } catch (error) {
