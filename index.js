    ...
    extractStatusMentions(text) {
        ...
        for (let [type, regex] of Object.entries(this.statusWords)) {
            ...
            while ((statusMatch = regex.exec(text)) !== null) {
                ...
                if (closestName) {
                    statuses.push({ name: closestName, status: type.charAt(0).toUpperCase() + type.slice(1) });
                }
            }
        }
        return statuses;
    ...
    detectCycles(text) {
        ...
        for (let [type, regex] of Object.entries(this.cycleWords)) {
            if (regex.test(text)) {
                ...
                if (!targetName && names.length > 0) targetName = names[0];
                if (targetName) {
...
(async function () {
    ...
    try {
        ...
        context.registerExtension({
            ...
            onEnable: () => {
                console.log('🧬 OV включен');
                OV_STATE.data.settings.enabled = true;
                OV_STATE.save();
            },
            onDisable: () => {
                console.log('🧬 OV выключен');
                OV_STATE.data.settings.enabled = false;
                OV_STATE.save();
            }
        });
    } catch (e) {
        console.error('OV: ошибка регистрации', e);
    }
